...
function displayInfo(type, which) {
	...
	if (type == "economy") {
		...
		out = `<div style="display: flex; gap: 5px; align-items: center"><span class="big"><b>${which}</b></span>${e[0].reduce((t, v) => `${t}<img src="images/goods/${goods[v].file}.png" draggable="false" style="vertical-align: middle"> `, "")}</div><p>${e[1]}</p>`;
	}
	if (type == "good") {
