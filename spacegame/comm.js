...
function displayComm (which, sfxFlag = true) {
	...
	if (!which && document.getElementById('commScreen').style.zIndex > 0) {hideComm(); return};
	...
	if (commPicker < 2) {
		console.log("commPicker: " + commPicker);
		let msgs = callQueue.length;
		if (msgs) {
			if (commPicker == 1.1 && currentCall > 0) currentCall--;
			if (commPicker == 1.2 && currentCall < msgs - 1) currentCall++;
...
function chooseComm (which) {
	let section = document.getElementsByClassName('commSection');
	for (let i of times(section.length)) section[i].style.display = (which == section.length - i) ? 'block' : 'none';

	document.getElementById('commScreen').style.zIndex = 6;
	let tab = document.getElementsByClassName('commTab');
	...
	document.getElementById('messagesButton').style.filter = (which < 2 && callQueue.length > 0) ? "hue-rotate(225deg) brightness(2)" : "";
	document.getElementById('messagesButton').style.opacity = (callQueue.length > 0) ? 1 : 0.2;
...
function addCall (callProto, insertFlag = false, preText = "", postText = "") {
	...
	if (insertFlag) callQueue.splice(currentCall + 1, 0, call);
	else callQueue.unshift(call);
...
function openCall (index = 0) {
	console.log(`Opening call ${index}...`);
	console.log(callQueue[index]);
	currentCall = index;
	const s = person[callQueue[index].speaker];
	...
	if (s) {
		out += `<div style='overflow:auto'>
				<img src='images/people/${s.image}.png' class='speaker' style='filter: hue-rotate(${s.color}deg) brightness(${s.brightness})' draggable='false'>
				<span class='big'><b>${s.name}</b></span><br>
				<span class='reduced'>${s.title}<br>${s.org}</span>
			</div>`;
		s.contact = time.full;
	...
	switch (callQueue[index].choice) {
		...
		case 4:		// (choice 0)/(choice 1)/.../end call/reject		(LocaleDonating)
			mission.find(v => v.id == callQueue[index].mission).choices.forEach((v, i) => {out += `<button class='commButton' type='button' onclick='proceedCall(${i})'>${v}</button> `});
...
function proceedCall (variant = 0) {
	...
	if (variant == "locale") addCall('setComm' in callQueue[currentCall] ? m.commData[callQueue[currentCall].setComm] : m.comm, true);
...
function rejectCall (flag) {
	...
	if ([1, 2, 3, 4].includes(callQueue[currentCall].choice) && m) {	// Reject avail. if choice = 1, 2, or 4
		if ('reject' in m && m.reject.length > 0) {
			...
			console.log(`Rejecting mission "${m.summary}" for ${person[m.client].name}...`);
			removeMission(m);		
		}
	}
	endCall();
...
function contactPerson (pID, shipFlag) {
	...
	const typewriter = (textArr) => timeouts.push(setTimeout(_ => {screen.innerHTML += textArr.shift(); if (textArr.length > 0) typewriter(textArr)}, 800));
	timeouts = [];
	...
	if (mClient) call = mClient.contact;
	// Call person normally (if not busy, sleeping, or despising you)
		...
		if ((!sleeping || (p.mood > 69 && callspace > (101 - p.mood) * 4)) && p.mood > 24 && !busy) {
			preText = "";
			if (sleeping) preText = `${rnd(["You woke me", "Oh, I was alseep", "<i>Yawn</i> Hello"])}. `;
			if (callback) preText += p.mood > 60 ? rnd([`Did you think of something else?`, `What is it?`, `Again? I'll still be here tomorrow!`]) : p.mood < 40 ? `Stop wasting my time.` : rnd([`I don't have all day for your calls.`, `We just spoke!`, `Once more, yes?`]);
			else if (callspace < 720 + rnd(500)) preText += p.mood > 60 ? `Nice to hear from you.` : p.mood < 40 ? `Oh, it's you.` : `Hello again.`;
			else preText += p.mood > 60 ? `Long time no see.` : `I remember you.`;
			...
			console.log(`DEBUG Default Conversation: Currently I don't have much to say.<br>The last time you contacted me was <b>${callspace}</b> hours ago.<br>My callback has been calculated as: <b>${callback}</b> (Mood: ${p.mood}).`);
			...
			if (p.mood > 39 && p.location == here) {
				...
				console.log(`Adding "same location" character mission...`)
				console.log(m);
